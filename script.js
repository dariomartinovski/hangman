...
const spaces = document.querySelectorAll(".line");

letterBtn.forEach(item => {
    item.addEventListener("click",e => {
        if(!item.classList.contains("wrong") && !item.classList.contains("correct")){
            if(word.includes(item.innerHTML)){  //if the word has the clicked letter
                for(let i=0;i<5;i++){
                    if(word[i] === item.innerHTML){ //checking if there are more same letters in the word, because include finds the first, same indexOf()
                        spaces[i].textContent = item.innerHTML; 
                    }
                }
                item.classList.add("correct"); //changes the background of the button to green
            }
            else{
                stickman[mistakes].setAttribute("style","display:block;"); // draw the stickman
                mistakes++;
...
function startGame(){
    for(let i=0;i<26;i++){
        ...
        letter.textContent = String.fromCharCode(65+i); //65 ascii is upper 'A'
...
function generateWord(){
    let index = Math.floor(Math.random()*5757);
    return (fiveLetterWords[index]).toUpperCase();
}

function hideStickman(){
    stickman.forEach(item => {
